/** Quarter-wave 16.16 sine table constructed by client.bin 0x116964. */
const SINE = [
...
// 0x1194e4 / 0x11959c use truncated quarter-wave indices in radians 16.16.
const QUARTER = 102943;
const HALF = 205887;
const THREE_QUARTERS = 308830;
const TURN = 411774;
export const nativeSine = (angle: number): number => {
  if (angle < 0) angle += TURN;
  if (angle > 411773) angle %= TURN;
  if (angle < QUARTER) return SINE[Math.trunc(angle * 256 / QUARTER)];
  if (angle < HALF) return SINE[256 - Math.trunc((angle - QUARTER) * 256 / QUARTER)];
  if (angle < THREE_QUARTERS) return -SINE[Math.trunc((angle - HALF) * 256 / QUARTER)];
  return -SINE[256 - Math.trunc((angle - THREE_QUARTERS) * 256 / QUARTER)];
};
export const nativeCosine = (angle: number): number => {
  angle = Math.abs(angle);
  if (angle > 411773) angle %= TURN;
  if (angle < QUARTER) return SINE[256 - Math.trunc(angle * 256 / QUARTER)];
  if (angle < HALF) return -SINE[Math.trunc((angle - QUARTER) * 256 / QUARTER)];
  if (angle < THREE_QUARTERS) return -SINE[256 - Math.trunc((angle - HALF) * 256 / QUARTER)];
  return SINE[Math.trunc((angle - THREE_QUARTERS) * 256 / QUARTER)];
};
